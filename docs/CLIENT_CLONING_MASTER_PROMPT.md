...
- **The Google Sheets CRM schema** (`Leads` 17 cols, `Appointments` 20 cols, `Communication Log` 9 cols — see `docs/CRM_SHEET_SCHEMA.md`)
...
- **The owner-controlled reschedule-notification logic** (Workflow 13 — fires when the owner checks `Notify Customer = TRUE` on a `Scheduled` row, sends a YES/NO confirmation SMS, and resets the checkbox — plus Workflow 10's `confirm_yes`/`confirm_no` reply handling, `Reschedule Status`/`Reschedule Attempts` tracking, and the 2-attempt automated-rescheduling cap)
...
10. **Reschedule/Cancel** — inbound SMS → keyword-classifies reschedule vs. cancel vs. opt-out vs. `confirm_yes`/`confirm_no` vs. irrelevant. Reschedule/cancel: finds the customer's nearest upcoming `Scheduled` appointment by phone → updates `Status`/`Notes` → replies to the customer → alerts the owner. `confirm_yes`/`confirm_no` (**added 2026-06-12**, the reply-handling half of Workflow 13's owner-controlled reschedule flow): matches against appointments with `Reschedule Status = Pending Customer Confirmation`. On YES: sets `Reschedule Status = Confirmed`, logs the reply, and emails the owner a confirmation. On NO: increments `Reschedule Attempts`, sets `Reschedule Status = Customer Requested Different Time` (or `Manual Follow-Up Required` once `Reschedule Attempts >= 2`, which also stops further automated rescheduling), replies to the customer, logs the reply, and emails/SMS-alerts the owner per `CONFIG`. Standalone opt-out keywords (`STOP`, `UNSUBSCRIBE`, etc.) are routed to silent suppression — **never auto-replied to**.
...
13. **Appointment Reschedule Notifier** — **redesigned 2026-06-12** from an automatic mismatch-detection trigger to an owner-controlled checkbox. Owner workflow: edit `Appt Date`/`Appt Time` in the Appointments tab as many times as needed, then check `Notify Customer = TRUE`. On its hourly run, the workflow picks up every `Scheduled` row with `Notify Customer = TRUE`, sends the customer a YES/NO reschedule-confirmation SMS for the current `Appt Date`/`Appt Time`, sets `Reschedule Status = Pending Customer Confirmation`, updates the `Notified Appt Date`/`Notified Appt Time` columns, clears `Reminder 24h`/`Reminder 2h` so Workflow 09 sends fresh reminders, resets `Notify Customer = FALSE`, logs the SMS via the CRM Adapter, and emails the owner. The customer's YES/NO reply is handled by Workflow 10 (see §4.1.10) — this design eliminates the premature/duplicate-notification risk of the old mismatch trigger, since the owner explicitly decides when notification is sent regardless of how many times the appointment was edited first.
...
| 13 — Appointment Reschedule Notifier | Customer (always, via SMS, when owner checks `Notify Customer` — not gated) + Owner (`OWNER_EMAIL`/`OWNER_PHONE`) | Customer: SMS. Owner: Email | Owner SMS via toggle `SMS_ALERTS_ENABLED` |
...
| 10 — Reschedule/Cancel | `test_workflow` with pinned inbound SMS for all paths: reschedule-found, cancel-found, not-found, a standalone opt-out keyword (e.g., `"STOP"`, confirm **zero** automated reply), plus `confirm_yes` and `confirm_no` (x2, to verify the `Reschedule Attempts >= 2` escalation to `Manual Follow-Up Required`) against a row with `Reschedule Status = Pending Customer Confirmation`. |
...
| 13 — Appointment Reschedule Notifier | Run twice on a pinned `Scheduled` row: (1) edit `Appt Date`/`Appt Time` and set `Notify Customer = TRUE` → confirm customer YES/NO SMS, owner email, `Notified Appt Date`/`Notified Appt Time` updated, `Reminder 24h`/`Reminder 2h` cleared, `Reschedule Status = Pending Customer Confirmation`, `Notify Customer` reset to `FALSE`, and a Communication Log entry via the CRM Adapter; (2) immediately re-run with no further changes → confirm zero items detected (since `Notify Customer` is now `FALSE`) and no duplicate send. |
