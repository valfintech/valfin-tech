...
|---|---|---|---|
| **Owner phone** | `+18575261499` | 04, 06 (set by user), 07, 08 (synced programmatically from 04), 10 (`OWNER_PHONE` constant in `Build Reply Plan`), **11 (`OWNER_PHONE` constant in the file's top-level constants — also the SMS recipient in `Send Health Alert SMS`)** | Set once in 04's `Build Alert Message`; for 08, 10, and 11, either repeat the "read live value + `update_workflow setNodeParameter`" pattern (see ROADMAP "New-workflow owner-phone setup" decision) or hand-edit each `OWNER_PHONE`/`Build Alert Message` constant |
| **Twilio "from" number** | `+18889839308` | 02, 03, 04, 05, 06, 09 (`FROM` constant in `Build Reminder Batch`), 10 (`FROM` constant in `Build Reply Plan`, and hardcoded `from` in `Send Not Found Reply`), **11 (`TWILIO_FROM_NUMBER` constant)** | Find-and-replace across every Twilio node's `from` field and every `FROM`/`from`/`TWILIO_FROM_NUMBER` constant in Code nodes |
| **Company name** | `Valfin Tech` | 02 (`Build Confirmation Request` system prompt), every customer-facing SMS template across 03/05/06/09/10 (workflow 11's alert is operator-facing only — its copy doesn't need rebranding, but still references "Workflow 09"/"Workflow 05" by number, which will differ in the cloned instance — see note below) | Find-and-replace in every hardcoded SMS string and AI system prompt |
| **Google Sheet ID** | `1MxmJouteZhi1K_-KOwgBlJtBFAXtm2G_0H555otTHBQ` | Every `googleSheets` node's `documentId.value` (workflows 01, 05, 06, 07, 08, 09, 10, **11 — `CRM_SPREADSHEET_ID` constant, both `Get All Appointments` and `Get All Leads (Health Check)` nodes**) | Replace in every `documentId: {__rl: true, mode: 'id', value: '...'}` block — easiest done via `update_workflow setNodeParameter` per node, or bulk find-replace on exported JSON before import |
...
6. **Import workflows 07, 08, 09, 11** → set Sheet ID, owner phone, Twilio "from," schedule timezone offsets → publish (11 also needs its `CRM_SPREADSHEET_ID` pointed at the same Sheet as the rest, and its alert-copy workflow-number references updated if 05/09 are renumbered in the new instance — see §3 above)
...
- ~~System-health monitoring workflow~~ — **done 2026-06-07.** `11_system_health_monitor` (`U6t0b7M6lN8eA1JO`) is live: a daily check that reads live CRM data freshness and texts the *vendor/operator* (never the client) if a scheduled workflow looks like it silently stopped running. See §3 above for its per-client config values and the README/PROJECT_STATUS entries for test results
