...
| Reschedule check | Hourly on the hour | Workflow 13 | Cadence-only, not timezone-sensitive — detects owner-initiated `Appt Date`/`Appt Time` changes (vs. `Notified Appt Date`/`Notified Appt Time`) and notifies the customer promptly after a manual reschedule |
