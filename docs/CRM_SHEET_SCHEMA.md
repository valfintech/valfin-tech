...
| `Notify Customer` | **Added 2026-06-12, converted 2026-06-13 from a checkbox to a dropdown** (`ONE_OF_LIST`, strict/reject-input, values `No` / `Send Update`, default `No`). The owner's trigger for sending a reschedule notice. Owner workflow: edit `Appt Date`/`Appt Time` (as many times as needed), verify, then set this dropdown to `Send Update`. On its next hourly run, Workflow 13 picks up any `Scheduled` row where this is `Send Update`, sends the customer a YES/NO reschedule-confirmation SMS for the **current** `Appt Date`/`Appt Time`, sets `Reschedule Status = Pending Customer Confirmation`, updates `Notified Appt Date`/`Notified Appt Time`, resets `Reminder 24h`/`Reminder 2h`, and resets this dropdown back to `No` — so the owner can select `Send Update` again later for another reschedule cycle without creating duplicate notifications. Replaces the V1.1-original automatic mismatch-detection trigger (which risked notifying customers prematurely or multiple times while the owner was still adjusting the appointment) |
