...
| **No email fallback for owner alerts (workflows 04/07/08 are SMS-only)** — identified 2026-06-10 | Client gets zero alerts during their own Twilio toll-free verification window (can take days) | Add a Gmail-OAuth2 (native node, `resource: "message"`/`operation: "send"`/`emailType: "html"`) email branch to `04 Hot Lead Alert` (and ideally 07/08), mirroring the pattern proven in `Valfin — Website Lead Capture`. Requires editing the off-limits client workflows — pick up in a session scoped/authorized for that. See `docs/CLIENT_SYSTEM_LESSONS_LEARNED.md` |
